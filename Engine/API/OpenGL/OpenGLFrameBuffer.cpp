...
namespace Engine
{
	...
	void OpenGLFrameBuffer::CreateTextureAttachment()
	{
		this->Bind();
		...
		mTextureAttachment = CreateRef<OpenGLTexture2D>(mWidth, mHeight, texture);
	
		this->Unbind();
	}
	
	void OpenGLFrameBuffer::CreateRenderBuffer()
	{
		this->Bind();
